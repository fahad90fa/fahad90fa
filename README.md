# 👋 Hi, I'm Fahad Yousaf
🚀 Full-Stack | MERN Developer | Open Source Enthusiast

I build secure, scalable and modern web apps using the latest technologies.  
Love clean UI, optimized backend, and smooth user experiences.

---

### 💻 Tech Stack
**Frontend:** React.js, Next.js, Tailwind CSS, Redux  
**Backend:** Node.js, Express.js, REST APIs, GraphQL  
**Database:** MongoDB, Firebase, PostgreSQL, Supabase  
**DevOps:** Docker, Nginx, CI/CD (GitHub Actions), Vercel, AWS  

---

### 📊 GitHub Stats
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Fahad-Yousaf&show_icons=true&theme=tokyonight)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=masterjee&layout=compact&theme=tokyonight)

---

### 🚀 Current Focus
- 🔐 Secure MERN apps (JWT, Encryption)
- ⚡ Performance Optimization
- ☁️ Cloud deployment (Vercel, AWS)
- 💬 Freelance / Collaboration

---

### 🌐 Connect With Me
[![Portfolio](https://img.shields.io/badge/Portfolio-000?style=for-the-badge&logo=vercel&logoColor=white)](https://yourportfolio.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/yourname)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:you@example.com)
